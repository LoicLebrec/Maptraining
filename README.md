...
- Demo (génère des GPX dans samples/) :

```bash
python scripts/demo.py
```

- Tests (génère des fichiers temporaires dans samples/test_outputs/) :

```bash
python tests/test_maptraining.py
```
...
├── scripts/                  # Scripts utilitaires (demo)
│   └── demo.py
├── tests/                    # Tests et validations
│   └── test_maptraining.py
├── samples/                  # Fichiers GPX d'exemple
│   └── test_outputs/          # Sorties temporaires de test
...
Voir le guide dans docs/CONTRIBUTING.md et le démarrage rapide dans docs/QUICKSTART.md.
